...
import com.code4a.library.R;
import com.code4a.library.utils.DensityUtil;

/**
 * 扇形进度条
 * Created by bruce on 11/6/14.
 * update by code4a on 15/12/15.
 */
public class ArcProgress extends View {
    ...
    private RectF pgRectF = new RectF();
    ...
    private final int default_shadow_color = Color.rgb(93, 23, 20);
    private final int default_finished_color = Color.rgb(252, 21, 10);
    private final int default_unfinished_color = Color.rgb(116, 116, 116);
    private final int default_text_color = Color.rgb(120, 143, 160);
    ...
    public ArcProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        default_text_size = DensityUtil.sp2px(getResources(), 8);
        min_size = (int) DensityUtil.dp2px(getResources(), 100);
        default_text_size = DensityUtil.sp2px(getResources(), 10);
        default_suffix_text_size = DensityUtil.sp2px(getResources(), 8);
        default_suffix_padding = DensityUtil.dp2px(getResources(), 1);
        default_suffix_text = "";
        default_bottom_text_size = DensityUtil.sp2px(getResources(), 8);
        default_stroke_width = DensityUtil.dp2px(getResources(), 3);
    ...
    protected void initByAttributes(TypedArray attributes) {
        shadowStrokeColor = attributes.getColor(R.styleable.ArcProgress_arc_shadow_color, default_shadow_color);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        bgRectF.set(strokeWidth / 1f, strokeWidth / 1f, width - strokeWidth / 1f, MeasureSpec.getSize(heightMeasureSpec) - strokeWidth / 1f);
        pgRectF.set(strokeWidth / 1f, strokeWidth / 1f, width - strokeWidth / 1f, MeasureSpec.getSize(heightMeasureSpec) - strokeWidth / 1f);
        shadowRectF.set(strokeWidth / 0.5f, strokeWidth / 0.5f, width - strokeWidth / 0.5f, MeasureSpec.getSize(heightMeasureSpec) - strokeWidth / 0.5f);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        paint.setStrokeWidth(strokeWidth);
        paint.setStrokeCap(Paint.Cap.ROUND);
        canvas.drawArc(bgRectF, startAngle, arcAngle, false, paint);

        paint.setColor(finishedStrokeColor);
        paint.setStrokeWidth(2*strokeWidth);
        paint.setStrokeCap(Paint.Cap.SQUARE);
        canvas.drawArc(pgRectF, finishedStartAngle, finishedSweepAngle, false, paint);

        paint.setColor(shadowStrokeColor);
        paint.setStrokeWidth(strokeWidth);
        paint.setStrokeCap(Paint.Cap.SQUARE);
        canvas.drawArc(shadowRectF, finishedStartAngle - strokeWidth/4f, finishedSweepAngle + strokeWidth/2f, false, paint);
